...
def test_logic_cache_lru():
    ...
    executor_u = LogicExecutor(
        ctx=cache, stats_factory=UniformStatsFactory(), cache=False
    )
    executor_d = LogicExecutor(
        ctx=cache, stats_factory=DenseStatsFactory(), cache=False
    )
    ...
    # Passing in the same (prgm, bindings),
    ...
    # Case 2 : Same prgm, bindings,
    # but diff statsfactory [DenseStatsFactory] here  - Expected MISS
    plan_1_d = Plan(
        (
            Query(Alias("out"), Table(Literal(data_1), (i, j))),
            Produces((Table(Alias("out"), (i, j)),)),
