...
def test_logic_cache_first():
    ...
    executor = LogicExecutor(ctx=cache)
    ...
    #Initiating the cache with a prgm binding pair
    plan_2 = Plan((
    ...
    print("\n Cached the first kernel for given (prgm,binding) pair")

    #Case 1 : Same (prgm,binding) pair
    ...
    executor(sim_plan_1)
    assert len(cache.cache) == 1
    print("\n HIT on the cached kernel for this (prgm,binding) pair")
    
    # Case 2 : Diff (prgm, binding) pair
    # Passing a different program so new addition to the cache
    executor(plan_2)
    assert len(cache.cache) == 2
    print("\n MISS, creating a new entry for the new (prgm,binding) pair")

    # Case 3 : Diff (prgm, binding) pair 
    ...
    print(" MISS, creating a new entry for the new (prgm,binding) pair")
...
def test_logic_cache_lru():
    ...
    executor_u = LogicExecutor(ctx=cache, stats_factory=UniformStatsFactory())
    executor_d = LogicExecutor(ctx=cache, stats_factory=DenseStatsFactory())
    ...
    #Initiating the cache with a (prgm, bindngs) pair
    plan_1_u = Plan((
    ...
    prgm_key = list(cache.cache.keys())[0]
    assert len(cache.cache[prgm_key]) == 1
    ...
    assert len(cache.cache[prgm_key]) == 1
    print("Cache hit for the (prgm,bindings)->stats combination")

    #Case 2 : Same prgm, bindings, but diff statsfactory [DenseStatsFactory] here
    plan_1_d = Plan((
        Query(Alias("out"), Table(Literal(data_1), (i, j))),
        Produces((Table(Alias("out"), (i, j)),))
    ))
    executor_d(plan_1_d)
    assert len(cache.cache[prgm_key]) == 2
    print("MISS since diff stats lead to a new entry in cache")

    #Case 3 : Exceeding max depth
