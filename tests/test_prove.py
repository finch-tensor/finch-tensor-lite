...
def test_rule_idempotent_unique():
    # max(a, a, b) => max(a, b)
    a, b = L("a"), L("b")
    expr = Call(L(ffunc.max), (a, a, b))
    result = rule_idempotent_unique(expr)
    assert isinstance(result, Call)
    assert result.op == L(ffunc.max)
...
def test_rule_associative_flatten():
    ...
    a, b, c = L("a"), L("b"), L("c")
    expr = Call(L(ffunc.max), (a, Call(L(ffunc.max), (b, c))))
    result = rule_associative_flatten(expr)
    assert result == Call(L(ffunc.max), (a, b, c))
...
def test_rule_disjoint_flat_single_min_max():
    ...
    a, b = L("a"), L("b")
    expr = Call(L(ffunc.min), (a, Call(L(ffunc.max), (a, b))))
    result = rule_disjoint_flat_single_min_max(expr)
    # Since 'a' is in both min args and max args (not disjoint), max is removed
    assert result == Call(L(ffunc.min), (a,))

    # Should not match when disjoint
    c = L("c")
    expr = Call(L(ffunc.min), (a, Call(L(ffunc.max), (b, c))))
    result = rule_disjoint_flat_single_min_max(expr)
    assert result is None
...
def test_rule_disjoint_flat_pair_max_min():
    ...
    a, b, c = L("a"), L("b"), L("c")
    expr = Call(L(ffunc.max), (Call(L(ffunc.min), (a, b)), Call(L(ffunc.min), (a, c))))
    ...
    # Should not match when disjoint
    d = L("d")
    expr = Call(L(ffunc.max), (Call(L(ffunc.min), (a, b)), Call(L(ffunc.min), (c, d))))
    result = rule_disjoint_flat_pair_max_min(expr)
    assert result is None
...
def test_rule_disjoint_flat_pair_min_max():
    ...
    a, b, c = L("a"), L("b"), L("c")
    expr = Call(L(ffunc.min), (Call(L(ffunc.max), (a, b)), Call(L(ffunc.max), (a, c))))
    ...
    # Should not match when disjoint
    d = L("d")
    expr = Call(L(ffunc.min), (Call(L(ffunc.max), (a, b)), Call(L(ffunc.max), (c, d))))
    result = rule_disjoint_flat_pair_min_max(expr)
    assert result is None
