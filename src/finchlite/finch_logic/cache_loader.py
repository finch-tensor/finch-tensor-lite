...
from .nodes import LogicStatement, Alias
...
class LogicCacheFirst(LogicLoader):
    ...
    def __call__(self, prgm : LogicStatement, bindings : dict[Alias, TensorFType], stats = None):
...
class LogicCacheLRU(LogicLoader):
    ...
    def __call__(self,prgm:LogicStatement,bindings:dict[Alias, TensorFType], stats = None):
        ...
        kernels = self.cache[prgm_key] #getting all the kernels for the prgm and bindings
