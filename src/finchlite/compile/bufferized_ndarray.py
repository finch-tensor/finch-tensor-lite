...
class BufferizedNDArrayAccessorFType(FinchTensorFType):
    ...
    def lower_increment(self, ctx, tns: ntn.Stack, val):
        obj = tns.obj
        ...
        if obj.tns.dirty_bit and self.op.val is overwrite:
            increment_call = asm.Call(
                asm.Literal(InitWrite(tns.type.fill_value)),
                (asm.Load(obj.tns.buf_s, lowered_pos), increment_call),
            )

        ctx.exec(asm.Store(obj.tns.buf_s, lowered_pos, increment_call))
