...
from ...algebra import resize_if_smaller, scansearch
        ...
        return asm.Block(
            (
                asm.Assign(
                    ...
                    asm.Call(
                        asm.Literal(resize_if_smaller),
                        (
                            level_fields.ptr_s,
                            asm.Call(asm.L(add), (pos_stop, asm.L(self.p_t(1)))),
    ...
    def level_unfurl(
        ...
        def seek_fn(ctx, ext):
            ...
            return asm.Block(
                (
                    asm.If(
                        asm.Call(asm.L(operator.lt), (asm.Load(idx_s, q), start)),
                        asm.Block(
                            (
                                asm.Assign(
                                    q,
            ...
            body=lambda ctx, ext: lplt.Sequence(
                head=lambda ctx, idx: lplt.Stepper(
                    preamble=lambda ctx: asm.IfElse(
                        asm.Call(asm.L(operator.lt), (q, q_stop)),
                    ...
                    next=lambda ctx: asm.Block(
                        (
                            asm.Assign(
                                q,
                                asm.Call(asm.L(add), (q, asm.L(self.p_t(1)))),
                            ),
                        )
                    ),
                    seek=seek_fn,
