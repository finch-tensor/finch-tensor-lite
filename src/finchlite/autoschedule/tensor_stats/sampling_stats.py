...
class SamplingStatsFactory(BaseTensorStatsFactory["SamplingStats"]):
    ...
    def _mapjoin_join(self, op:FinchOperator, *join_args:SamplingStats)->SamplingStats:
        ...
        for arg in join_args[1:]:
            result,result_order = _outer_multiply(result,result_order,arg.sketch,list(arg.index_order))
    ...
    def _mapjoin_union(self, op:FinchOperator, *union_args:SamplingStats):
        ...
        result_shape = tuple(int(base_stats.dim_sizes[f]) for f in base_stats.index_order)
        ...
        for arg in union_args:
            ...
            expanded = _expand_sketch_to(arg.sketch,list(arg.index_order),
                                         list(base_stats.index_order),base_stats,)
        ...
        if len(union_args)>=2:
            ...
            for arg in union_args[1:]:
                inter,inter_order = _outer_multiply(inter,inter_order,arg.sketch,list(arg.index_order),)
            
            inter_expanded = _expand_sketch_to(inter,inter_order,list(base_stats.index_order),
                                               base_stats,)
    ...
    def aggregate(self, op:FinchOperator, init: Any| None, reduce_indices : tuple[Field,...], stats:SamplingStats):
        ...
        if is_identity(op,stats.fill_value):
            new_sketch = np.sum(stats.sketch,axis=reduce_axes) if reduce_axes else stats.sketch.copy()
    ...
    def reorder(self,stats:SamplingStats,reorder_indices:tuple[Field,...])->SamplingStats:
        ...
        new_sketch = _reorder_to(stats.sketch.sopy(),old_order,list(reorder_indices))
...
class SamplingStats(NumericStats):
    ...
    def __init__(self, tensor:Any, fields:tuple[Field,...],sample_prob:float=0.5,estimator : str = "uj1",mask_fn=None,masks_ref:dict|None=None):
        ...
        if hasattr(val,"to_numpy"):
            ...
            shape = tuple(int(self.dim_sizes[f]) for f in fields)
