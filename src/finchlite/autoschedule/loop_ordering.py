...
def _get_conjunctive_and_disjunctive_inputs(
    ...
    match expr:
        case Aggregate(arg=arg):
            return _get_conjunctive_and_disjunctive_inputs(
                arg, stats_factory, stats_bindings, cache, disjunct_branch
            )
        case Reorder(arg=arg) | Relabel(arg=arg):
            return _get_conjunctive_and_disjunctive_inputs(
                arg, stats_factory, stats_bindings, cache, disjunct_branch
            )
        case MapJoin(Literal() as op_node, args):
            ...
            for arg in args:
                ...
                arg_conjuncts, arg_disjuncts = (
                    _get_conjunctive_and_disjunctive_inputs(
                        arg,
                        stats_factory,
                        stats_bindings,
                        cache,
...
def _prefix_lookups(
    ...
    rel_conjuncts = [
        _stats_with_false_fill(stats_factory, stat)
    ...
    if not rel_disjuncts or prefix_set.issubset(conjunct_index_set):
        loop_stats = _merge_prefix_stats(stats_factory, ffuncs.and_, rel_conjuncts)
    elif not rel_conjuncts:
        loop_stats = _merge_prefix_stats(stats_factory, ffuncs.or_, rel_disjuncts)
    else:
        disjunct_union = _merge_prefix_stats(
            stats_factory, ffuncs.or_, rel_disjuncts
        )
        loop_stats = _merge_prefix_stats(
            stats_factory,
            ffuncs.and_,
...
def loop_order_cost(
    ...
    cache: dict[object, TensorStats] = {}
    conjunct_stats, disjunct_stats = _get_conjunctive_and_disjunctive_inputs(
        expr, stats_factory, stats_bindings_2, cache
    )
    cost = 0.0
    for j in range(1, len(loop_order) + 1):
        cost += _prefix_lookups(
