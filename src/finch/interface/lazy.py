...
from ..finch_logic.interpreter import FinchLogicInterpreter
...
def lazy(arr) -> LazyTensor:
    ...
    returned as-is. Otherwise, it creates a LazyTensor representation of the input
    array.
...
def expand_dims(
    ...
    for d in axis:
        offset[d] = 1
    offset = list(itertools.accumulate(offset))
...
def reduce(
    ...
    *,
    ...
    data: Aggregate | Reorder = Aggregate(
        Immediate(op),
        Immediate(init),
...
def prod(arr: LazyTensor, dims: tuple[int, ...]) -> LazyTensor:
    ...
    dtype: Optional[dtype]
        data type of the returned array. If ``None``, the returned array must have the
        same data type as ``x``, unless ``x`` has an integer data type supporting a
        smaller range of values than the default integer data type (e.g., ``x`` has an
        ``int16`` or ``uint32`` data type and the default integer data type is
        ``int64``). In those latter cases:
        ...
        computing the sum (rationale: the ``dtype`` keyword argument is intended to help
        prevent overflows). Default: ``None``.
    ...
    **Special Cases**
    ...
    For both real-valued and complex floating-point operands, special cases must be
