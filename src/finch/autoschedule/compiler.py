    ...
    def __call__(
        ...
        match ex:
            case MapJoin(Literal(op), args):
                return ntn.Call(
                    ntn.Literal(op),
                    tuple(self(arg, table_vars, field_relabels) for arg in args),
                )
            ...
            case Reorder(arg, _):
                return self(arg, table_vars, field_relabels)
            case Field(_) as f:
...
def compile_pointwise_logic(
    ...
    code = ctx(ex, table_vars, field_relabels)
        ...
        match ex:
            ...
            case Query(
                ...
                (rhs, rhs_idxs) = compile_pointwise_logic(
                    Relabel(arg, idxs_1), list(loop_idxs), table_vars, field_relabels
                )
            ...
            case Query(
                ...
                (rhs, rhs_idxs) = compile_pointwise_logic(
                    arg, list(idxs_2), table_vars, field_relabels
                )
            ...
            case Plan(bodies):
                func_block = ntn.Block(
                    tuple(
                        self(body, table_vars, dim_size_vars, field_relabels)
                        for body in bodies
                    )
                )
                return ntn.Module(
...
def record_tables(
    ...
    return processed_root, table_vars, dim_size_vars, tables, field_relabels
...
class LogicCompiler:
    ...
    def __call__(self, prgm: LogicNode) -> tuple[ntn.NotationNode, dict[Alias, Table]]:
        prgm, table_vars, dim_size_vars, tables, field_relabels = record_tables(prgm)
        lowered_prgm = self.ll(prgm, table_vars, dim_size_vars, field_relabels)
